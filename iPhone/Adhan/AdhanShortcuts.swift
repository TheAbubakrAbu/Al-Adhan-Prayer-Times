...
struct WhenIsPrayerIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        ...
        guard let prayers = settings.todayFullPrayerList(), !prayers.isEmpty else {
            let message = "Prayer times aren’t available yet. Open Al-Adhan to refresh."
...
struct CurrentPrayerIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        ...
        let message = "No current prayer determined yet. Open Al-Adhan to refresh prayer times."
        return .result(value: message, dialog: IntentDialog(stringLiteral: message))
    }
}
...
struct NextPrayerIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        ...
        let message = "No upcoming prayer found. Open Al-Adhan to refresh prayer times."
        return .result(value: message, dialog: IntentDialog(stringLiteral: message))
    }
}
